...
Альтернативные реализации (код готов):
- 📦 VK через Wappi → mcp-vk-wappi/
- 📦 MAX через Wappi → mcp-max-wappi/

Следующие шаги:
1. Получить credentials для каналов
2. Обновить .env на сервере
3. Настроить webhooks в платформах
4. Создать n8n workflows (BAT IN/OUT)
5. Тестирование и дебаг

Сервер API: 45.144.177.128
...
│  ┌────────────────────────────────────────────────────┐    │
│  │              n8n Workflows                          │    │
│  │  BAT IN {Channel} → Processing → BAT OUT {Channel}  │    │
│  └────────────────────────────────────────────────────┘    │
│                           │                                 │
│                           ▼                                 │
│  ┌────────────────────────────────────────────────────┐    │
...
| GET | `/health` | Health check | - |
| GET | `/info` | Server info | - |
| POST | `/api/submit` | Отправить форму | - |
| POST | `/api/quiz` | Отправить квиз | - |
| POST | `/api/callback` | Заявка на обратный звонок | - |
| POST | `/api/contact` | Контактная форма | - |
| POST | `/webhook/form` | Webhook для Tilda и др. | API Key |
...
```javascript
{
  ...
  email: "ivan@mail.ru",
  form_id: "quiz_battery",
  form_name: "Квиз по аккумуляторам",
  message: "Комментарий",

  // Квиз
  quiz_answers: [
    { question: "Марка", answer: "iPhone" },
    { question: "Модель", answer: "12 Pro" }
  ],
...
## План реализации каналов

### Фаза 1: Avito (в процессе)

- [x] MCP Server развёрнут
- [x] Токен получается
- [ ] Подписать webhook
- [ ] Обновить BAT IN Avito
- [ ] Обновить BAT OUT Avito
- [ ] Интеграционный тест
...
| Провайдер | Стоимость | Сложность | Ограничения |
|-----------|-----------|-----------|-------------|
| WAPPI | ~3000₽/мес | Низкая | Неофициальный |
| 360dialog | $$ | Средняя | Официальный партнёр |
| WhatsApp Business API | $$$ | Высокая | Официальный |
...
Для создания нового канала используй структуру из `mcp-avito/`:

```bash
# Создать папку для нового канала
mkdir -p mcp-{channel}
...
## Чеклист нового канала

При добавлении нового канала:

- [ ] Создать папку `mcp-{channel}/`
- [ ] Реализовать `{channel}_client.py`
- [ ] Адаптировать `app.py`
- [ ] Создать `config.py` с настройками
- [ ] Настроить `docker-compose.yml`
- [ ] Задеплоить на 45.144.177.128
- [ ] Создать/обновить `BAT IN {Channel}` в n8n
- [ ] Создать/обновить `BAT OUT {Channel}` в n8n
- [ ] Обновить Message Router
...
| Workflow | Канал | Направление |
|----------|-------|-------------|
| BAT IN Telegram | Telegram | Входящие |
| BAT OUT Telegram | Telegram | Исходящие |
| BAT IN Avito | Avito | Входящие |
| BAT OUT Avito | Avito | Исходящие |
| BAT IN VK | VK | Входящие |
| BAT OUT VK | VK | Исходящие |
| BAT IN WhatsApp | WhatsApp | Входящие |
| BAT OUT WhatsApp | WhatsApp | Исходящие |
| BAT IN MAX | MAX | Входящие |
| BAT OUT MAX | MAX | Исходящие |
| BAT Message Router | Все | Маршрутизация |
