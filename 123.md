...
### 2.1 MessagerOne Server (155.212.221.189)
...
|--------|------|--------|----------|
| avito-listener | 8775 | ✅ | WebSocket listener для Avito |
| mcp-whatsapp-arceos | 8769 | ✅ | WhatsApp Baileys |
| avito-messenger-api | 8766 | ✅ | Avito HTTP API |
| max-bot-api | 8768 | ✅ | MAX Bot API |
| telegram-bot-api | 8761 | ✅ | Telegram Bot API |
| telegram-user-api | 8762 | ⏳ | Ждёт credentials |
| redis | 6379 | ✅ | Cache |

**Проверка avito-listener:**
```bash
curl http://155.212.221.189:8775/health
# {"service":"avito-listener","accounts_total":0,"accounts_connected":0,"accounts":[]}
```

**Логи avito-listener:**
```bash
ssh root@155.212.221.189 "journalctl -u avito-listener -f"
```

**Рестарт avito-listener:**
```bash
ssh root@155.212.221.189 "systemctl restart avito-listener"
```

**Конфигурация avito-listener:**
```bash
ssh root@155.212.221.189 "cat /opt/avito-listener/.env"
# N8N_WEBHOOK_URL=https://n8n.n8nsrv.ru/webhook/avito/incoming
# POSTGRES_URL=postgresql://postgres:eldoleado@185.221.214.83:6544/postgres
# PORT=8775
```
...
**Таблицы ELO:**
- `elo_t_channel_accounts` — аккаунты каналов (sessid хранится здесь)
- `elo_t_events` — события
- `elo_t_dialogs` — диалоги
- И другие elo_* таблицы

---

### 2.3 RU Server (45.144.177.128)
...
**Сервисы:**
- Neo4j: 7474/7687
- mcp-avito, mcp-vk, mcp-max, mcp-form
- api-android: 8780

---

### 2.4 Finnish Server (217.145.79.27)
...
### ELO_Client_Resolve
1. Искать: `WHERE external_id = sender_id AND channel_type = 'avito'`
2. Если не найден → создать клиента:
   - `name` = sender_name
...
|--------|-----|------|----------|
| MessagerOne | 155.212.221.189 | root | Mi31415926pSss! |
| n8n | 185.221.214.83 | root | Mi31415926pS |
| RU | 45.144.177.128 | root | Mi31415926pSss! |
| Finnish | 217.145.79.27 | root | Mi31415926pSss! |
...
4. ✅ Полный цикл работает: WebSocket → Android → n8n webhook

### Осталось:
1. ⏳ n8n: Normalize Message → добавить client_name
2. ⏳ n8n: ELO_Client_Resolve → создание клиента по sender_id
3. ⏳ Реализовать другие каналы (MAX, Telegram)

---

*Последнее обновление: 2025-12-24 14:35 MSK*
