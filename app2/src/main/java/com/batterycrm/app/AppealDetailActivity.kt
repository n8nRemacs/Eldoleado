...
import com.batterycrm.app.data.database.entities.AppealEntity
import com.batterycrm.app.data.database.entities.MessageEntity
import com.batterycrm.app.databinding.ActivityAppealDetailBinding
import com.batterycrm.app.ui.AppealInfoBottomSheet
import com.batterycrm.app.viewmodel.AppealDetailUiState
import com.batterycrm.app.viewmodel.AppealDetailViewModel
...
class AppealDetailActivity : AppCompatActivity() {
    ...
    private val viewModel: AppealDetailViewModel by viewModels {
        AppealDetailViewModel.Factory(
            BatteryCRMApplication.instance.repository,
            appealId
        )
    }
    ...
    private fun updateUIFromCache(appeal: AppealEntity, messages: List<MessageEntity>) {
        ...
        // Show AI response if available and input is empty (but not right after sending)
        if (!messageSentRecently && binding.messageInput.text.isNullOrBlank() && !appeal.aiResponse.isNullOrBlank()) {
