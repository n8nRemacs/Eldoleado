...
import com.batterycrm.app.AppealEventBus
import com.batterycrm.app.AppealUpdateEvent
...
class BatteryCRMMessagingService : FirebaseMessagingService() {
    ...
    private fun handleCacheInvalidation(type: String, appealId: String?, draftText: String? = null) {
        serviceScope.launch {
            try {
                val repository = BatteryCRMApplication.instance.repository
                when (type) {
                    "new_appeal" -> {
    ...
    companion object {
        private const val TAG = "BatteryCRMFCM"
    }
